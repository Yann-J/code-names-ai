...
# ---------- runtime: nginx (static) + uvicorn (API/live) + PWA -------------
...
RUN apt-get update \
    ...
    && useradd -r -u 10001 -m -d /home/app -s /usr/sbin/nologin app \
    && mkdir -p /cache /tmp/nginx \
        /var/lib/nginx/body /var/lib/nginx/proxy \
        /var/lib/nginx/fastcgi /var/lib/nginx/uwsgi /var/lib/nginx/scgi \
    && chown -R app:app /cache /tmp/nginx /var/lib/nginx
...
COPY --chown=app:app deploy/nginx/default.conf /app/deploy/nginx/default.conf
...
# nginx :8000 serves the PWA from disk; uvicorn :8001 handles /api and /live only.
